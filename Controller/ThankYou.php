...
class ThankYou extends ThankYou_parent
{
    ...
    public function getEmosCode(&$aEmos)
    {
        ...
        $sEventName = $this->getConfig()->getConfigParam('itr_analytics_emos_extorderevent');
        if ($this->getConfig()->getConfigParam('itr_analytics_emos_extorder') && $sEventName) {
            // Credit card type.
            $sCcType = 'n.a.';
            // Payment type.
    ...
    public function getGoogleTagEvents(&$aGoogleTagEvents)
    {
        ...
        if($this->getConfig()->getConfigParam('itr_analytics_gtag_ecommerce')){
            $aBasket = [];
            foreach ($this->getBasketProducts() as $aProdData) {
